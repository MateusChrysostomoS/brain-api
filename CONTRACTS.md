...
| `META_ES_COEXISTENCE_FEATURE_TYPE` | `"whatsapp_business_app_onboarding"` | The Embedded Signup `extras.featureType` value that turns on the Coexistence flow (onboarding an existing WhatsApp Business app user, vs. a brand-new number); echoed read-only via `GET /doctor/onboarding`'s `embedded_signup.coexistence_feature_type` (§16.2) so the portal can decide whether to offer that option. Empty ⇒ not offered. Not a secret |
...
See `docs/CHECKPOINT_coexistence.md` for the "WhatsApp Coexistence onboarding" round
(2026-08-09) that added `META_ES_COEXISTENCE_FEATURE_TYPE`, `embedded_signup.
coexistence_feature_type`, `meta_graph.subscribe_app_to_waba`, and the
`waba_subscribe_failed`/`no_phone_number_id` `error_code` vocabulary documented below.
