...
- [[_COMMUNITY_secretaria_internal.py|secretaria_internal.py]]
...
Cohesion: 0.16
Nodes (12): get_entitlements(), AsyncSession, Entitlements endpoint (CONTRACTS.md §3.1, stripe-billing-entitlements skill).  `, Return the resolved entitlement state for the authenticated tenant.      `requir, EntitlementOut, ProductsOut, Pydantic v2 schemas for the entitlements vertical (CONTRACTS.md §3.1).  The re, Per-product access flags (CONTRACTS.md §3.1 `products`).      `precheck` <- en (+4 more)
...
Cohesion: 0.11
Nodes (18): anamneses(), anamnesis_detail(), appointments(), doctor_me(), patients(), AsyncSession, Doctor (tenant) endpoints (RBAC task, Part 1B) — `auth-jwt-multitenant` skill., List the tenant's anamnesis records (brain-api -> precheck `/api/v1/doctor/anamn (+10 more)

### Community 35 - "test_onboarding.py"
Cohesion: 0.06
Nodes (16): _new_tenant(), AsyncSession, services/onboarding.py — pure state-machine logic (STATE-MACHINE + AUTH build)., Both hint groups present -> the numero_em_outro_bsp check wins (checked first)., `record_attempt` never commits — a rollback after it must undo everything., prior_api=='yes' takes priority even when fb_page ALSO signals a blocker., test_derive_initial_state_priority_prior_api_wins_over_fb_page(), test_map_error_to_blocker_numero_priority_over_admin() (+8 more)
...
Cohesion: 0.28
Nodes (8): BoundLogger, get_logger(), Structured logging setup using structlog.  JSON output in non-dev environments,, Blank any value whose key ends in `_encrypted` or looks secret-bearing., Configure structlog + stdlib logging. Safe to call more than once., Return a bound structlog logger., redact_secrets(), setup_logging()
...
Cohesion: 0.40
Nodes (5): 2.1 `POST /auth/token` — login (public), 2.1a `POST /auth/refresh` — rotate the session (public, rate-limited), 2.1b `POST /auth/logout` — revoke a refresh token (public), 2.2 `GET /auth/me` — current identity (protected), 2. Authentication

### Community 69 - "brain-api"
Cohesion: 0.50
Nodes (3): brain-api, Documentação — manter em dia (obrigatório), graphify

### Community 71 - "12. Doctor (tenant) API (RBAC round) — roles `tenant_owner` / `tenant_staff`"
Cohesion: 0.50
Nodes (4): 12.1 brain-api → secretaria internal data connection (service-to-service), 12.2 secretarIA doctor-hub auth — hub token + introspection (billing/auth round), 12.3 secretarIA → PreCheck patient handoff (`POST /internal/precheck-handoff`), 12. Doctor (tenant) API (RBAC round) — roles `tenant_owner` / `tenant_staff`
...
Cohesion: 0.17
Nodes (18): _FakeResponse, _install_fake_meta_httpx(), _install_fake_provisioning_httpx(), Exception, MonkeyPatch, SimpleNamespace, test_connect_whatsapp_network_error_is_connection_error(), test_connect_whatsapp_ok_conflict_and_error_status_mapping() (+10 more)
...
Cohesion: 0.25
Nodes (6): AdminUserCreateIn, Create a user in any tenant, with any role (admin tooling, Part 1A).      Pass, Minimum composition: length is enforced by the Field; require letter + digit, A platform admin has no tenant; a tenant user must name one., create_user(), Create a user in any tenant with any role. Email is stored lower-cased.      4
...
Cohesion: 0.13
Nodes (19): _install_fake_stripe_httpx(), Point services.billing at fake Stripe settings + a recording fake httpx client., Register a lead (first card) and return its intent_id., Persisted order is normalized (plan first, add-ons deduped) regardless of what, Defense-in-depth: an add-on with no configured Stripe price is rejected even, _register(), test_checkout_session_happy_path(), test_checkout_session_not_pending_conflict() (+11 more)
