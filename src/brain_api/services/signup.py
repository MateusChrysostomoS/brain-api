"""Cold-signup service: lead capture, Stripe Checkout, onboarding poll + provisioning.

The bridge from a public "stranger fills a form" lead to a paying, logged-in tenant with
NO human in the loop (auth-jwt-multitenant + stripe-billing-entitlements skills). Three
steps, in order:

1. `create_signup_intent`               — persist the lead + catalog selection
   (pending_payment); 409s early if the email is already a user so provisioning can never
   fail on a duplicate AFTER payment.
2. `create_checkout_session_for_intent` — open a Stripe Checkout Session for that intent.
3. `provision_tenant_from_intent`       — the ONLY tenant/user/entitlement writer on this
   path, called from the Stripe webhook apply path (`services.billing.apply_stripe_event`)
   on `checkout.session.completed` with `metadata.kind == "signup_intent"`. Mirrors
   `scripts/seed_dev.py`'s bootstrap order (Tenant -> flush -> User -> Entitlement); the
   initial entitlement state is materialized directly from the intent's `catalog_ids`
   (not the `customer.subscription.created` event, which can arrive before the tenant
   exists and be unresolvable) — later `subscription.*` events reconcile it via the
   `stripe_customer_id` fallback `_entitlement_for_event` already implements.

`get_onboarding_status` is the ONLY place that mints the onboarding token: the plaintext
must reach the browser, so only a synchronous poll (never the webhook) can hand it back.
Only its SHA-256 hash (`core.security.hash_refresh_token`, the same primitive
`RefreshToken` uses) is ever persisted, rotated on every poll until exchanged exactly once
...
async def create_signup_intent(session: AsyncSession, payload: SignupIntentCreate) -> SignupIntent:
    """Persist one pending-payment signup intent.

    409 if a user with this email already exists (case-insensitive) — checked NOW so
    provisioning can never fail on a duplicate after the buyer has already paid.
    """
...
# --- 3. Provisioning (the ONLY writer; webhook apply path) ---------------------------
...
async def provision_tenant_from_intent(
    ...
    *,
    ...
    """Materialize the tenant + owner user + entitlement a paid signup intent implies.
    ...
    existing = await session.scalar(select(User).where(User.email == intent.email))
    if existing is not None:
        # Raced: something else registered this email between intent creation and this
        # webhook delivery. Fail the intent WITHOUT raising — the webhook must still ack
        # and mark the Stripe event processed (billing.apply_stripe_event commits the
        # ProcessedStripeEvent row around this call regardless of the outcome). Checked
        # BEFORE any insert so the same commit can't leave an orphan tenant behind.
        intent.status = "failed"
        intent.failure_reason = "email_already_registered"
        logger.warning("signup_provision_email_conflict", intent_id=str(intent.id))
        return
    ...
    logger.info("signup_provisioned", intent_id=str(intent.id), tenant_id=str(tenant.id))
