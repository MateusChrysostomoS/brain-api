...
- Catalog integrity (`services/catalog.py`): every declared id/limit key is internally
  consistent and the reserved slot is excluded from what an admin may assign.
...
def test_assignable_plan_ids_excludes_reserved_bronze_2():
    assert catalog.PLAN_SECRETARIA_BRONZE_2 not in catalog.ASSIGNABLE_PLAN_IDS
    assert catalog.ASSIGNABLE_PLAN_IDS == frozenset(
        {
            catalog.PLAN_FREE,
            catalog.PLAN_PRECHECK,
            catalog.PLAN_SECRETARIA_FERRO,
            catalog.PLAN_SECRETARIA_BRONZE_1,
...
def test_compute_limits_addon_grants_are_additive():
    limits = catalog.compute_limits(
        catalog.PLAN_SECRETARIA_BRONZE_1, {catalog.ADDON_REACTIVATION_PACK: True}
    )
    # base reminders=200 + reactivation_pack grant=200
    assert limits[catalog.LIMIT_REMINDERS] == 400
    assert limits[catalog.LIMIT_HSM_PROACTIVE] == 200
...
def test_compute_entitlement_state_combo():
    ...
    assert state["limits"][catalog.LIMIT_REMINDERS] == 400


def test_compute_entitlement_state_addon_override_grants_limit():
    state = catalog.compute_entitlement_state(
        catalog.PLAN_SECRETARIA_FERRO, {catalog.ADDON_MULTI_PROFESSIONAL: True}
    )
    # ferro base professionals=1 + multi_professional grant=1
...
def test_is_entitled_status_gate_denies_everything(status):
    ...
    assert is_entitled(ent, catalog.TIER_BRONZE_1) is False


def test_is_entitled_tiers_are_cumulative():
    ent = _ent(catalog.PLAN_SECRETARIA_BRONZE_1, status="active")
    assert is_entitled(ent, catalog.TIER_FERRO) is True
    assert is_entitled(ent, catalog.TIER_BRONZE_1) is True
    assert is_entitled(ent, catalog.TIER_BRONZE_2) is False


def test_is_entitled_plan_without_tier_denies_ferro():
    ent = _ent(catalog.PLAN_PRECHECK, status="active")
    assert is_entitled(ent, catalog.TIER_FERRO) is False
...
async def test_admin_patch_chain_plan_then_addons_then_limits(client):
    ...
    assert body["limits"][catalog.LIMIT_REMINDERS] == 400
    ...
    assert ent["secretaria_tier"] == catalog.TIER_BRONZE_1
...
async def test_admin_patch_rejects_unassignable_or_unknown_plan(client):
    ...
    for plan in ("nonsense", catalog.PLAN_SECRETARIA_BRONZE_2):
        resp = await client.patch(
            f"/admin/tenants/{tenant_b_id}/entitlements",
            headers=_bearer(admin_token),
            json={"plan": plan},
        )
        assert resp.status_code == 422, f"{plan}: {resp.text}"
